...
/**
 * Sign arbitrary data with HaLo chip
 */
export async function signMessageWithHalo(
  message: string,
  keySlot: number = 1
): Promise<string> {
  try {
    ...
    const digest = messageHash.slice(2); // Remove '0x' prefix

    // Sign with HaLo chip via Bridge
    console.log(`Signing message with key slot ${keySlot}...`);
    const result = await execBridgeCommand({
      name: "sign",
      keyNo: keySlot,
      digest,
    ...
    // Construct signature (bridge returns raw.r, raw.s, raw.v)
    const sig = result.signature.raw || result.signature;
    ...
    console.error("Error signing message with HaLo:", error);
